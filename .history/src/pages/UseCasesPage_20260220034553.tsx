        ...
        <div className="container px-4 md:px-6 mx-auto">
          <div className="space-y-8 md:space-y-12">
            {useCases.map((useCase, index) => (
              <div
                key={index}
                className="relative group overflow-hidden rounded-2xl shadow-2xl transition-all duration-500 bg-background"
              >
                {/* Full Width Centered Image */}
                <div className="relative w-full flex items-center justify-center p-6 md:p-8 lg:p-12">
                  <img
                    src={useCase.image}
                    alt={useCase.title}
                    className="rounded-2xl shadow-xl mx-auto max-w-5xl w-full object-cover h-[400px] md:h-[500px] lg:h-[600px] transition-transform duration-700 group-hover:scale-[1.02]"
                  />
                </div>
              </div>
            ))}
